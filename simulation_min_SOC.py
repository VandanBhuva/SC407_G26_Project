...
HE = []
end_index = df.shape[0] // (48 * 2)
for day in range(end_index):
...
df['Min_SOC'] = pd.to_numeric(df['Min_SOC'])

# Truncate HE to match the length of the DataFrame index
HE = HE[:len(df)]

# Assign HE to the 'HE' column
df['HE'] = HE
...
class Experiment():
    ...
    def run(self):
            ...
            for hour in range(0, self.n_hours):
                ...
                for soc_bin in range(0, self.n_divisions_for_soc):
                    ...
                    if self.index_to_time_of_day_dict[hour] in [17,18,19,20,21,22,23,0,1,2]:
                        ...
                        if action == 0:
                            self.soc_of_evs, charging_load_index = self.v_get_soc_and_charging_load(soc_bin, self.soc_of_evs, soc_div_index, status_evs, soc_reduction_for_evs, hour)
                            charging_load = self.charging_soc_mw_addition_to_demand_per_time_unit_per_ev * charging_load_index.sum()

                        elif action == 1:
                            self.soc_of_evs, discharging_load_index = self.v_get_soc_and_discharging_load(soc_bin, self.soc_of_evs, soc_div_index, status_evs, soc_reduction_for_evs, hour)
                            discharging_load = self.discharging_soc_mw_reduction_from_demand_per_time_unit_per_ev * discharging_load_index.sum()
                        else:
                            self.soc_of_evs = self.v_get_soc_from_driving(soc_bin, self.soc_of_evs, soc_div_index, status_evs, soc_reduction_for_evs, hour)
        ...
        #print(self.Q)
        # Save statistics
        np.save('average_runs/' + id_run + '_reward_list2.npy', self.reward_list)
        np.save('average_runs/' + id_run + '_average_list2.npy', self.average_list)
        np.save('average_runs/' + id_run + '_Q2.npy', self.Q)
        np.save('average_runs/' + id_run + '_PAR_list2.npy', self.PAR_list)
        np.save('average_runs/' + id_run + '_max_list2.npy', self.max_load_list)
        np.save('average_runs/' + id_run + '_Q_change_list2.npy', self.Q_change_list)
        np.save('average_runs/' + id_run + '_Q_list2.npy', self.Q_list)
    ...
    #     else:
    #         return soc_of_evs, 0
    def get_soc_and_charging_load(self, division, soc_of_evs, soc_div_index, status_evs, soc_reduction_for_evs, hour):
        if soc_div_index == division:
            if status_evs == 1:
                if soc_of_evs < self.max_soc_allowed:  # Check if SOC is below the maximum allowed SOC
                    new_soc = min(self.max_soc_allowed, soc_of_evs + self.charging_soc_addition_per_time_unit_per_ev)
                    return new_soc, 1
                else:
                    return soc_of_evs, 0
            elif status_evs == 0:
                if soc_of_evs > self.min_soc_allowed[self.index_to_time_of_day_dict[hour]]:  # Check if SOC is above the minimum allowed SOC
                    new_soc = soc_of_evs - soc_reduction_for_evs #self.alberta_average_demand[self.index_to_time_of_day_dict[hour]]
                    return new_soc, 0
                else:
                    return soc_of_evs, 0
        else:
    ...
    #     else:
    #         return soc_of_evs
    def get_soc_from_driving(self, division, soc_of_evs, soc_div_index, status_evs, soc_reduction_for_evs, hour):
        if soc_div_index == division:
            if status_evs == 1:
                return soc_of_evs
            elif status_evs == 0:
